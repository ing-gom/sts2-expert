...
1. **단독 평가가 기준.** 등급은 네 자체 분석(효과 사실 + 평가 프레임워크)으로만 산정한다. 커뮤니티 티어 리스트류의 외부 평가 데이터는 이 번들에 포함되어 있지 않으며, 어디선가 발견하더라도 평가에 사용하지 않는다.
...
| 파일 | 내용 |
|---|---|
| `data/cards_catalog.json` | **마스터 카탈로그** (~1,117장, 강화판 포함). id/title(한국어)/cost/star_cost/type/rarity/character/axes/vars/description/builds. `game_version` 필드가 버전 출처. 커뮤니티 티어 필드는 단독 평가 원칙에 따라 이 번들에서 제거됨 |
...
**평가 전 레포 루트의 `eval_criteria.md`를 읽어 함께 적용한다.** 효율·스케일링·덱 압축·시너지·신규 메커니즘(부식/관통/내구도)·조건부 페널티 등 평가 기준과 패치별 메타 맥락을 담은 전문가 기준집이다. 외부에서 흡수하는 것은 *평가 방법론과 메타*이지 티어 값이 아니므로, 등급은 여전히 네가 이 기준으로 직접 산정한다.

### 카드
1. **효과 사실 수집**: description·vars·cost·star_cost·axes·builds를 읽는다.
...
4. **축·아키타입 적합도**: axes + builds 필드로 어느 엔진의 keystone/연료인지. **엔진 집중 > 개별 카드 파워** ("범용 굿스터프" 과대평가 금지 — 개별 카드 파워 추종이 아키타입 집중보다 실측 성과가 나빴다).
5. **등급 확정**: S~D를 결정하고 근거를 적는다. `card_empirical_lift.json`에 인과 실측값이 있으면 맥락 노트에 함께 인용한다 (커버: Defect 일부).
...
### 등급 체계 (리포트 공통)
S(게임 체인저/빌드 코어) · A(매우 강함, 범용) · B(덱 따라 유용) · C(조건 맞을 때만) · D(거의 항상 함정). 정적 등급과 맥락 등급이 다르면 둘 다 표기 (예: "정적 S / 스타터 덱 C"). 등급은 전부 에이전트 자체 판정이다.
...
## 정보 정리
(1차 소스에서 수집한 효과·수치·축을 표로. 출처 파일 경로 명시)
